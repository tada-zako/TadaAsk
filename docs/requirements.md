# TadaWidget — 项目需求与架构文档

## 1. 项目定位

TadaWidget 是一款轻量级、可自托管的开源 AI 知识库助手。核心集成方式为 **Widget 优先**：通过一行 `<script>` 标签即可将具备 RAG 能力的对话组件嵌入任意静态站点或文档页面。
...
- 多项目统一管理仪表盘（一个 Project 对应一个站点部署）。
...
- 一个 **Project** 对应一个站点部署，可关联多个 **Source**。
